...
export default function FormularioEditarPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  
  // Utilização do Context global
  const { formularios, atualizarFormulario } = useFormularios();

  const [form, setForm] = useState(null);
  const [toast, setToast] = useState(false);

  useEffect(() => {
    // Procura o formulário correspondente no array do Context
    const encontrado = formularios.find((f) => f.id === parseInt(id));
    if (encontrado) {
      setForm({ ...encontrado });
    }
  }, [id, formularios]);
  ...
  function handleGuardar() {
    // Atualiza o formulário no Context com os dados modificados do estado local
    atualizarFormulario(parseInt(id), form);
...
// Componente auxiliar para radio buttons
